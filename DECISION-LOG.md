...
| 2026-09-21 | M5：五个页面按 §11 放在 `web/pages/`，但用 `st.navigation` + `st.Page` 显式注册（不用 legacy 自动发现） | §11 冻结了文件位置；Streamlit 官方建议的新写法可以照用，只需把页面路径显式写进入口 |
| 2026-09-21 | M5：共享只读模型放在 `web/app.py`，不新建 `web/readmodel.py` | §11 的 `web/` 只列了 app.py 与 pages/；页面保持直筒脚本，读档逻辑集中一处 |
| 2026-09-21 | M5：新增 `.streamlit/config.toml` 把服务绑到 `127.0.0.1` | 实测 `streamlit run` 默认监听 0.0.0.0，而确认页是全系统唯一放行口且无鉴权——绑到公网接口等于把 T5 威胁摊开；§0.10 取更保守一方 |
| 2026-09-21 | M5：`PushEvent` 增加 `facts` 段（引擎判定行），卡片分「引擎判定 / 引擎数字 / 评论」三段 | §9 要求卡片含「每码 stance」与「规则触发原因」，它们既不是数字也不是 agent 评论，混进任一段都是错的标签 |
| 2026-09-21 | M5：日报卡不再展示「浮动盈亏」，改报引擎真算得出的量（总值、目标仓位合计 bps、订单数、待确认、风控否决） | 展示模式没有券商回报，也没有逐日市价序列；造一个盈亏数字会直接违反「数字一律 engine 出」 |
| 2026-09-21 | M5：页面测试用 `st.testing.v1.AppTest` 无头渲染，不起浏览器也不启服务器 | 官方推荐的测法，确定且可进 CI；`streamlit run` 只做一次人工启动校验 |
| 2026-09-21 | M5：`wecom.post_markdown` 的测试 monkeypatch 掉 `requests.post`，不发真实请求 | §15.11 禁联网；只验请求体形状与「无 webhook 即拒绝」的分支 |
